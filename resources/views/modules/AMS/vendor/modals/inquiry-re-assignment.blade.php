...
<form id="inquiryReAssignmentForm">
    ...
    <div class="modal-header border-bottom border-2 py-2 bg-secondary-subtle">
        <h5 class="modal-title" id="fullscreeexampleModalLabel">Re Assign Inquiry | {{ $inquiry->company_name.' '.$inquiry->source.' Inquiry' }}</h5>
            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <div class="alert alert-warning alert-border-left alert-dismissible fade show material-shadow" role="alert">
                        <i class="ri-alert-line me-3 align-middle"></i> <strong>Warning</strong> - This inquiry is assigned to {{ $inquiries_assigned_details['name'] }} and assigned date and time is: {{ $inquiries_assigned_details['assigned_on'] }}
                    </div>
                </div>
            </div>
                ...
                <div class="col-lg-3 col-md-6 mb-3">
                    ...
                    <div class="input-group">
                        <select class="form-control ms select2" name="agent_id" id="agent_id" data-placeholder="Select Agent">
                            ...
                            <option value="{{ $user->id }}" {{ ($user->id == $inquiry->inquiry_assigned_to) ? "selected" : "" }}>{{$user->name}}</option>
                            @empty
    ...
    <div class="modal-footer">
        ...
        <input type="hidden" name="inquiry_assigment_id" value="{{ $inquiry_assigment->id }}">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#inquiryReAssignmentForm').submit(function(event) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('crm.save-re-assign-inquiry') }}",
                data: $(this).serialize(),
