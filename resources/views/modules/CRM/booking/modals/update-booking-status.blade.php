...
<form id="updateTicketStatus">
    ...
    <div class="modal-header border-bottom border-2 py-2 bg-secondary-subtle">
        <h5 class="modal-title" id="fullscreeexampleModalLabel">Update Ticket Status | Booking Number: {{ $booking->booking_prefix . $booking->booking_number }}</h5>
       ...
       <div class="row g-4">
            ...
            <div class="col-lg-4 col-sm-6">
                <div class="form-check card-radio">
                    <input id="booking_status_{{ $key+1 }}" name="booking_status" type="radio" class="form-check-input" value="{{ $status->detail_number }}" {{ ($booking->booking_status == $status->detail_number) ? "checked" : ""; }}>
                    <label class="form-check-label" for="booking_status_{{ $key+1 }}">
...
<script>
    ...
    $('#updateTicketStatus').submit(function(event) {
        ...
        $.ajax({
            type: 'POST',
            url: "{{ route('crm.save-booking-status') }}",
            data: $(this).serialize(),
