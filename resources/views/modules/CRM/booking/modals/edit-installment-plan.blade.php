    ...
    <div class="modal-body">
        ...
        <table class="table table-bordered table-nowrap table-sm">
            ...
            <tbody>
                <tr>
                    ...
                    <td><span class="text-danger">{{ $booking->currency . ' ' . $booking->balance_amount }}</span>
            ...
            <tbody>
                @foreach ($booking->payments as $payment)
                    <tr>
                        ...
                        <td><span class="text-success">{{ $booking->currency . ' ' . $payment->reciving_amount }}</span>
                        </td>
                        <td><span class="text-danger">{{ $booking->currency . ' ' . $payment->remaining_amount }}</span>
                        </td>
        ...
        @else
            @if ($booking->installmentPlan->count() > 0)
                @foreach ($booking->installmentPlan as $installmentPlan)
                    <div class="row clearfix hotel_info mb-1 {{ $installmentPlan->is_received == 1 ? 'bg-success-subtle' : '' }}" id="installment_plan">

                        <div class="col-lg-4 col-md-6">
                            <label>Installment Number</label>
                            <input type="text" class="form-control"
                                name="installment[{{ $installmentPlan->id }}][installment_number]"
                                value="{{ $installmentPlan->installment_number }}" placeholder="Installment Number" readonly>
                        </div>
                        <div class="col-lg-4 col-md-6">
                            <label>Installment Due Date</label>
                            <input type="text" class="form-control {{ $installmentPlan->is_received == 1 ? '' : 'flatpickr due_date flatpickr-date' }}"
                                data-provider="flatpickr" autocomplete="off" id="dueDate_{{ $installmentPlan->id }}"
                                name="installment[{{ $installmentPlan->id }}][due_date]" value="{{ $installmentPlan->due_date }}"
                                placeholder="Installment Due Date" required {{ $installmentPlan->is_received == 1 ? 'readonly' : '' }}>
                        </div>

                        <div class="col-lg-4 col-md-6">
                            <label>Installment Amount</label>
                            <input type="number" class="form-control {{ $installmentPlan->is_received == 1 ? '' : 'installment_amount' }}"
                                name="installment[{{  $installmentPlan->id  }}][amount]" value="{{ $installmentPlan->amount }}"
                                placeholder="Installment Amount" step="0.01" required="" {{ $installmentPlan->is_received == 1 ? 'readonly' : '' }}>
                        </div>
    ...
    $(document).ready(function() {
        ...
        $('#installmentForm').submit(function(event) {
            ...
            if (totalInstallmentAmount !== balance_amount) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('crm.update-installment-plan') }}",
                    data: $(this).serialize(),
