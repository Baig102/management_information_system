...
<form id="paymentForm">
    ...
    <div class="modal-header border-bottom border-2 py-2 bg-secondary-subtle">
        <h5 class="modal-title" id="fullscreeexampleModalLabel">Add Other Charges Payment | Booking Number: {{ $booking->booking_prefix . $booking->booking_number }}</h5>
    ...
    <div class="modal-body">
        {{-- <pre>
            {{ print_r($booking) }}
            {{ print_r($otherChargesPayments) }}
            {{ print_r($otherCharges) }}
        </pre> --}}
        ...
        <table class="table table-bordered table-nowrap table-sm">
            ...
            <tbody>
                ...
                @else
                <tr>
                    <td>{{ $otherCharges->charges_type }}</td>
                    <td><span class="text-primary">{{ $booking->currency . ' ' . number_format($otherCharges->amount, 2) }}</span></td>
                    <td><span class="text-success">{{ $booking->currency . ' ' . number_format($otherCharges->reciving_amount, 2) }}</span></td>
                    <td><span class="text-danger">{{ $booking->currency . ' ' . number_format($otherCharges->remaining_amount, 2) }}</span></td>
                    <td>{{ $otherCharges->comments }}</td>
                    <td>{{ $otherCharges->charges_at }}</td>
                </tr>
                @endif
            </tbody>
        </table>
        ...
        <table class="table table-bordered table-nowrap table-sm">
            ...
            <tbody>
                @if ($otherChargesPayments->count() <= 0)
                <tr class="text-center">
                    <td colspan="5">No Record Found</td>
                </tr>
                @else
                    @foreach ($otherChargesPayments as $oc_key => $other_charges)
                        <tr>
                            <th scope="row"> {{ $oc_key+1 }} </th>
                            <td><span class="text-success">{{ $booking->currency . ' ' . number_format($other_charges->reciving_amount, 2) }}</span> </td>
                            <td><span class="text-danger">{{ $booking->currency . ' ' . number_format($other_charges->remaining_amount, 2) }}</span> </td>
                            <td>
                                {{ $other_charges->payment_method }}
                            </td>
                            <td>{{ $other_charges->deposit_date }}</td>
                        </tr>
                    @endforeach
                @endif
            </tbody>
        ...
        @if ($otherCharges->payment_status === 1)
            ...
            <div class="row mb-3">
                <div class="col-lg-3 col-md-6 mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control amount" id="amount" name="amount" value="{{ $otherCharges->amount }}" placeholder="Enter Amount" readonly>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 mb-3">
                    ...
                    <div class="input-group">
                        <span class="input-group-text px-2 py-1"><i class="ri-money-pound-circle-line fs-5"></i></span>
                        <input type="number" class="form-control reciving_amount" id="reciving_amount" name="reciving_amount" value="" placeholder="Enter Reciving Amount" step="0.01" required max="{{ $otherCharges->remaining_amount }}">
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 mb-3">
    ...
    <div class="modal-footer">
        ...
        <input type="hidden" name="other_charges_id" id="" value="{{$otherCharges->id}}">
