    ...
    <div class="modal-body" id="hotel_details">
        ...
        <div class="row clearfix hotel_info mb-3" id="hotelInfo_{{ $hotel->id }}">
            ...
            <div class="col-lg-2 col-md-6 mb-3">
                ...
                <input type="text" class="form-control" name="hotel[{{$hotel->id}}][hotel_name]" value="{{ $hotel->hotel_name }}" placeholder="Hotel Name" required>
            </div>
            <div class="col-lg-2 col-md-6 mb-3">
                <label>Check in date<span class="text-danger"> *</span></label>
                <input type="text" class="form-control flatpickr check_in_date" id="checkInDate_{{$hotel->id}}" name="hotel[{{$hotel->id}}][check_in_date]" value="{{ $hotel->check_in_date }}" onchange="calculateAndDisplayDateDifference(this.id)" placeholder="Check in date" required>
            </div>
            <div class="col-lg-2 col-md-6 mb-3">
                <label>Check out Date<span class="text-danger"> *</span></label>
                <input type="text" class="form-control flatpickr check_out_date" id="checkOutDate_{{$hotel->id}}" name="hotel[{{$hotel->id}}][check_out_date]" value="{{ $hotel->check_out_date }}" onchange="calculateAndDisplayDateDifference(this.id)" placeholder="Check out date" required>
            </div>
            <div class="col-lg-2 col-md-6 mb-3">
                <label>Total nights<span class="text-danger"> *</span></label>
                <input type="text" class="form-control total_nights" id="total_nights_{{$hotel->id}}" name="hotel[{{$hotel->id}}][total_nights]" value="{{ $hotel->total_nights }}" placeholder="Total nights" readonly="" required>
            </div>
            <div class="col-lg-2 col-md-6 mb-3">
                <label>Meal Type<span class="text-danger"> *</span></label>
                <input type="text" class="form-control" name="hotel[{{$hotel->id}}][meal_type]" value="{{ $hotel->meal_type }}" placeholder="Meal Type" required>
            </div>
            <div class="col-lg-2 col-md-6 mb-3">
                <label>Room Type<span class="text-danger"> *</span></label>
                <input type="text" class="form-control" name="hotel[{{$hotel->id}}][room_type]" value="{{ $hotel->room_type }}" placeholder="Room Type" required>
            </div>
            <div class="col-lg-3 col-md-6 mb-3">
                <label>HCN <small>Hotel Confirmation Number</small></label>
                <input type="text" class="form-control" name="hotel[{{$hotel->id}}][hotel_confirmation_number]" value="{{ $hotel->hotel_confirmation_number }}" placeholder="HCN">
            </div>
            <div class="col-lg-2 col-md-6 mb-3">
                <label>Sale Cost{!! $required_check_sales_cost !!}</label>
                <input type="number" class="form-control hotel_sale_cost" name="hotel[{{$hotel->id}}][sale_cost]" value="{{ $hotel->sale_cost }}" placeholder="Sale Cost" step="0.01" {{ $filed_condition_sales_cost }}>
            </div>
            <div class="col-lg-2 col-md-6 mb-3">
            ...
            <div class="col-lg-2 col-md-6 mb-3">
                ...
                <input type="text" name="hotel[{{$hotel->id}}][deadline]" value="{{ $hotel->deadline }}" placeholder="DD-MM-YYYY" class="form-control flatpickr" data-provider="flatpickr" id="hotel_deadline_{{$hotel->id}}" autocomplete="off" required>
            </div>
            ...
            {{-- <div class="col-lg-2 mt-3">
                ...
                <button type="button" class="btn btn-danger btn-label waves-effect waves-light btn-soft-danger btn-sm clone_row mt-4"  onclick="remove_row('hotelInfo_{{ $hotel->id }}', {{ $hotel->id }})"><i class="ri-indeterminate-circle-line label-icon align-middle fs-16 me-2"></i>Remove</button>
            </div> --}}
            <div class="col-lg-1 mt-3">
                <input type="hidden" name="hotel[{{ $hotel->id }}][id]" value="{{ $hotel->id }}">
                <button type="button" class="btn btn-danger waves-effect waves-light btn-soft-danger btn-sm clone_row mt-3"  onclick="remove_row('hotelInfo_{{ $hotel->id }}', {{ $hotel->id }})"><i class="ri-indeterminate-circle-line align-middle fs-16 me-2"></i></button>
            </div>
...
<div class="modal-footer">
    <input type="hidden" name="hotel_count" id="hotel_details_count" value="{{ $booking->hotels->count()+1}}">
    <input type="hidden" name="booking_id" id="booking_id" value="{{ $booking->id }}">
    <a href="javascript:void(0)" class="btn btn-warning" id="addHotel" onclick="add_hotel_rows({{ $booking->is_full_package }})"><i class="ri-shield-user-fill me-1 align-middle"></i>Add</a>
...
<script>
        ...
        if (data_id != null) {
            ...
            confirmButtonText: 'Yes, delete it!'
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route("crm.delete-hotel-details", "") }}/' + hotel_id,
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
    ...
    $('#updateHotelDetails').submit(function(event) {
        ...
        $.ajax({
            type: 'POST',
            url: "{{ route('crm.update-hotel-details') }}",
            data: $(this).serialize(),
            success: function(response) {
                //console.log(response);
